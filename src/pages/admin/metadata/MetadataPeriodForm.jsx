...
const addEmperor = () => {
  const name = prompt("Nhập tên vị vua:");
  if (name) {
    const newEmp = { id: Date.now(), name, years: 'Chưa rõ', img: 'https://via.placeholder.com/150' };
  ...
  <div className="flex gap-3 items-center">
    <button onClick={() => navigate(-1)} className="px-5 py-2 border border-primary text-primary font-mono text-[10px] font-bold hover:bg-primary/5 transition-all uppercase flex items-center justify-center"> HỦY BỎ </button>
    <button onClick={() => {
      console.log("SAVE PERIOD:", form);
      alert("Đã lưu dữ liệu!");
    }}
    className="min-w-[160px] py-2.5 bg-primary text-white font-mono text-[10px] font-bold shadow-lg hover:bg-primary-container flex items-center justify-center gap-2 transition-all uppercase">      <span className="material-symbols-outlined text-sm">save</span>
          ...
          <div className="col-span-12 lg:col-span-8 space-y-10">
               ...
               <div className="grid grid-cols-2 gap-8 relative z-10">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex gap-2">
                       <input type="number" value={form.startYear} onChange={e => setForm({...form, startYear: e.target.value})} className="flex-1 bg-surface-low border border-outline-variant rounded p-2 text-sm outline-none focus:ring-1 focus:ring-primary" placeholder="1009" />
                       <select value={form.eraType} onChange={e => setForm({...form, eraType: e.target.value})} className="bg-surface-low border border-outline-variant rounded px-2 text-[10px] font-bold">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex gap-2">
                       <input type="number" value={form.endYear} onChange={e => setForm({...form, endYear: e.target.value})} className="flex-1 bg-surface-low border border-outline-variant rounded p-2 text-sm outline-none focus:ring-1 focus:ring-primary" placeholder="1225" />
            ...
            <section className="bg-white p-8 border border-outline-variant shadow-sm rounded-lg">
               ...
               <div className="space-y-6">
                  ...
                  <div className="space-y-1">
                    ...
                    <RichTextEditor value={form.description} onChange={(html) => setForm({ ...form, description: html })} placeholder="Viết về các thay đổi chính trị - xã hội..." />
                  </div>
               </div>
            </section>
            ...
            <section className="bg-white p-8 border border-outline-variant shadow-sm rounded-lg">
               <div className="flex justify-between items-center mb-8">
                  ...
                  <button onClick={addEmperor} className="font-mono text-[10px] font-bold text-primary hover:underline flex items-center gap-1">+ THÊM VỊ VUA</button>
               </div>
          ...
          <div className="col-span-12 lg:col-span-4 space-y-6">
            <div className="bg-primary/90 text-white p-6 shadow-xl rounded-sm">
              ...
              <div className="flex flex-col gap-2">
                <label className="text-[10px] font-bold uppercase opacity-70">Trạng thái thời kỳ</label>
                <select value={form.status} onChange={e => setForm({...form, status: e.target.value})} className="w-full bg-white/10 border border-white/20 rounded p-2 text-sm outline-none focus:bg-white/20 cursor-pointer">
