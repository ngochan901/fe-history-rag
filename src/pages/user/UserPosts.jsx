...
const UserPosts = () => {
  ...
  const handleToggleLike = (id) => {
    setArticleList(prevList => prevList.map(art => {
      if (art.id === id) {
        const currentlyLiked = art.isLiked;
        return {
          ...art,
          isLiked: !currentlyLiked,
          likes: currentlyLiked ? art.likes - 1 : art.likes + 1
        };
      }
      return art;
    }));
  ...
  // Helper để format số like (VD: 1200 -> 1.2k)
  const formatLikes = (num) => {
    return num >= 1000 ? (num / 1000).toFixed(1).replace(/\.0$/, '') + 'k' : num;
  };

  // Lấy dữ liệu từ State (articleList) thay vì biến tĩnh
  const featuredArt = articleList.find(a => a.featured);
  const regularArticles = articleList.filter(a => !a.featured);
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 border border-secondary/20 bg-white shadow-xl overflow-hidden rounded-sm group">
              <div className="lg:col-span-7 relative h-[400px] lg:h-auto overflow-hidden">
                <Link to={`/articles/${featuredArt.slug}`}>
                  <img className="w-full h-full object-cover grayscale-[0.3] group-hover:grayscale-0 transition-all duration-1000 scale-105 group-hover:scale-100 cursor-pointer" src={featuredArt.image} alt="Featured" />
                </Link>
              ...
              <div className="lg:col-span-5 p-12 flex flex-col justify-center bg-[#f7f4e9] border-l border-secondary/10">
                ...
                <Link to={`/articles/${featuredArt.slug}`}>
                  <h3 className="font-headline text-4xl text-on-surface mb-6 leading-tight italic font-bold hover:text-primary transition-colors cursor-pointer tracking-tight">{featuredArt.title}</h3>
                </Link>
                <p className="font-body text-base text-on-surface-variant mb-10 italic leading-relaxed">{featuredArt.excerpt}</p>
                <div className="flex items-center justify-between mt-auto">
                  <div className="flex items-center gap-6">
                    {/* Nút Like cho bài Featured */}
                    <button 
                      onClick={() => handleToggleLike(featuredArt.id)}
                      className={`flex items-center gap-2 font-mono text-[11px] font-bold transition-all active:scale-90 ${featuredArt.isLiked ? 'text-primary' : 'text-secondary/60 hover:text-primary'}`}
                    >
                      <span className="material-symbols-outlined text-[24px]" style={{ fontVariationSettings: featuredArt.isLiked ? "'FILL' 1" : "" }}>favorite</span>
                      {formatLikes(featuredArt.likes)}
                    </button>
                    <span className="text-secondary/60 font-mono text-[11px] font-bold uppercase flex items-center gap-2">
                      <span className="material-symbols-outlined text-[24px]">chat_bubble</span> {featuredArt.comments}
                    </span>
                  </div>
                  <Link to={`/articles/${featuredArt.slug}`} className="bg-primary text-white px-10 py-3 font-bold text-[10px] uppercase tracking-widest hover:brightness-110 transition-all shadow-md">KHÁM PHÁ</Link>
                </div>
              </div>
            </div>
          )}
            ...
            <article key={art.id} className="group bg-white border border-secondary/10 hover:border-primary/40 transition-all duration-500 hover:shadow-2xl flex flex-col">
              <div className="h-60 overflow-hidden relative">
                <Link to={`/articles/${art.slug}`}>
                  <img className="w-full h-full object-cover grayscale-[0.2] group-hover:grayscale-0 group-hover:scale-110 transition-transform duration-700" src={art.image} alt={art.title} />
                </Link>
              ...
              <div className="p-8 border-t border-secondary/5 flex flex-col flex-grow">
                ...
                <Link to={`/articles/${art.slug}`}>
                  <h4 className="font-headline text-2xl text-on-surface group-hover:text-primary transition-colors mb-5 italic font-bold leading-tight tracking-tight">{art.title}</h4>
                </Link>
                <p className="font-body text-sm text-on-surface-variant line-clamp-3 mb-8 italic leading-relaxed">{art.excerpt}</p>
                <div className="flex items-center justify-between pt-6 border-t border-outline-variant/10 mt-auto font-mono">
                  <div className="flex gap-6">
                    {/* NÚT LIKE CHO BÀI VIẾT TRONG GRID */}
                    <button 
                      onClick={() => handleToggleLike(art.id)}
                      className={`flex items-center gap-1.5 text-[11px] font-bold transition-all active:scale-90 ${art.isLiked ? 'text-primary' : 'text-secondary/70 hover:text-primary'}`}
                    >
                      <span className="material-symbols-outlined text-[20px]" style={{ fontVariationSettings: art.isLiked ? "'FILL' 1" : "" }}>favorite</span>
                      {formatLikes(art.likes)}
                    </button>
                    
                    <span className="flex items-center gap-1.5 text-[11px] font-bold text-secondary/70">
                      <span className="material-symbols-outlined text-[20px]">chat_bubble</span> {art.comments}
                    </span>
                  </div>
                </div>
              </div>
